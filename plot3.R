...
Emit_Point <- tapply(Point$Emissions, Point$year, mean)
Emit_NonPoint <- tapply(NonPoint$Emissions, NonPoint$year, mean)
Emit_OnRoad <- tapply(OnRoad$Emissions, OnRoad$year, mean)
Emit_NonRoad <- tapply(NonRoad$Emissions, NonRoad$year, mean)
...
labs(title = 'Average Emssion of PM2.5 for 4 types of source 
                                    in Baltimore City') +
